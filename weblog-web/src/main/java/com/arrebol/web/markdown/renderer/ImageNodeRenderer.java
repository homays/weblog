package com.arrebol.web.markdown.renderer;
...
public class ImageNodeRenderer implements NodeRenderer {
    ...
    public void render(Node node) {
        ...
        if (StrUtil.isNotBlank(imgTitle)) {
        ...
        if (Objects.nonNull(lastChild) && lastChild instanceof ImageAttributes) {
            ...
            if (Objects.nonNull(imageAttributes) && !CollUtil.isEmpty(imageAttributes.getAttributes())) {
                String width = imageAttributes.getAttributes().get(KEY_WIDTH);
                String height = imageAttributes.getAttributes().get(KEY_HEIGHT);
                sb.append(StrUtil.isBlank(width) ? "" : (" " + KEY_WIDTH + "=" + width + "\""));
                sb.append(StrUtil.isBlank(height) ? "" : (" " + KEY_HEIGHT + "=" + height + "\""));
            }
        }
        sb.append(">");
